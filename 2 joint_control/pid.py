...
import os
import sys
sys.path.append(os.path.join(os.path.abspath(os.path.dirname(__file__)), '..', '0 software_installation'))
...
class PIDController(object):
    ...
    def control(self, target, sensor):
        ...
        e = target-(sensor+self.y[0])

        const0 = self.Kp + self.Ki * self.dt + self.Kd/self.dt
        const1 = self.Kp + 2*self.Kd/self.dt
        const2 = self.Kd/self.dt

        self.u +=  const0*e - const1*self.e1 + const2* self.e2
